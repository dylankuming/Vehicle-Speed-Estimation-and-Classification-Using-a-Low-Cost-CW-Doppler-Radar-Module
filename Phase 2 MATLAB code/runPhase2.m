...
%% Set BIN file and direction paramaters
fileName    = '02_Phase2_uncontrol_30min_towards.bin';    % Teensy-generated binary file
direction = "towards";           % <-- set manually: "towards" or "away"


%% Input file and parameters
% explicitly set parameters telling MATLAB how the Teensy sliced up 
% the data and how to map each spectrogram column and row back into real
% time and frequency
fs          = 44.1e3;           % sampling rate on Teensy
frameLength = 4096;             % FFT length used on Teensy
overlap     = frameLength*0.5;    % 50% overlap
hopsize     = frameLength - overlap;


%% Load binary spectrogram data
fid = fopen(fileName, 'r');
raw = fread(fid, inf, 'single');  % all 32-bit floats
fclose(fid);

numFrames = numel(raw) / (frameLength/2); % Number of time frames captured (columns of 2048 bins)
P  = reshape(raw, frameLength/2, numFrames); % Reshape into [2048 bins × numFrames] matrix

%% Build time and frequency axes
T    = (0:numFrames-1) * (hopsize/fs); 
F   = (0:frameLength/2-1) * (fs/frameLength); 


%–– Doppler → speed (m/s → km/h)
lambda = 299792458 / 24e9;    % radar wavelength
v_mps  = (F * lambda) / 2;


%–– Crop to 10–70 km/h
mask  = (v_mps >= 10/3.6) & (v_mps <= 70/3.6);
P     = P(mask, :);
v_mps = v_mps(mask);
...
% Gap based parameters (1D in m/s) 
...
for col = 1:Cols
    ...
    % forces both speedsInCol and ampsInCol to be column vectors (Nx1)
    speedsInCol = speedsInCol(:); 
    ampsInCol   = ampsInCol(:);

%NEW--------------------------------------------
    % 1) Compute gaps between adjacent speeds
    d = diff(speedsInCol);
    

    % 2) Identify where gap > epsilon
    breakIdx = find(d > epsilon);

    % 3) Define cluster boundaries: starts at 1, splits at each breakIdx, ends at last point
    boundaries = [0; breakIdx; length(speedsInCol)];

    repVals = [];
    % 4) Loop over each candidate segment defined by boundaries
    for k = 1:length(boundaries)-1
        iStart = boundaries(k) + 1;
        iEnd   = boundaries(k+1);
        sc = speedsInCol(iStart:iEnd);    % speeds for this segment
        am = ampsInCol(iStart:iEnd);    % amplitudes for this segment (weights)
        clusterSize = numel(sc);

 %--------------------------------------------

        % Only accept this segment as a cluster if it has >= minPts points
        if clusterSize >= minPts


            % For visualisation only: store all cluster points
            allClusterTimes  = [allClusterTimes;  repmat(T(col), clusterSize, 1)];
            allClusterSpeeds = [allClusterSpeeds; sc(:)];
    
            % Representative value: arithmetic mean
    
            detectionVal = sum(sc .* am) / sum(am);
            ...
            allNoiseTimes  = [allNoiseTimes;  repmat(T(col), clusterSize, 1)];
            allNoiseSpeeds = [allNoiseSpeeds; sc(:)];
...
% Plot Gap-based Results in km/h
% Cluster points (green), noise points (blue), representative points (red)
figure('Name','Gap-based Clustering and Chosen Detections');
hold on; grid on;
...
title('Gap-based Clustering, Representative and Noise Detections');
...
figure('Name','Final Gap-based Result');
scatter(finalDetTimes, finalDetSpeeds*3.6, 5, 'r', 'filled');
xlabel('Time (s)'); ylabel('Speed (km/h)');
title('Multiple Detections per Column (Gap-based)');
% xlim([0,15]);
ylim([10,70]);
...
% Overlay all Gap-based detections on the same plot for reference.
scatter(finalDetTimes, finalDetSpeeds*3.6, 10, 'ks', 'filled', ...
    'DisplayName', 'Gap-based Detections');
...
% Overlay Gap-based detection points for comparison
scatter(finalDetTimes, finalDetSpeeds*3.6, 10, 'ks', 'filled',...
    'DisplayName', 'Gap-based Detections');
...
%% Vehicle Classification with Row-by-Row Detection Widththreshold_dB = -22;  % keep everything >= -22 dB
threshold_dB = -22;
...
for k = 1:numVehicles
    ...
    %-----------------------------------------------------------------------
    % NEW: Local normalization inside that speed-time window
    %   1) Extract all speed bins and time columns in bounding box
    localRegion = P(speedIdxRange, timeIdxRange);  

    %   2) Normalize that region by its own maximum:
    localMax = max(localRegion(:));
    if localMax > 0
        localNorm = localRegion / localMax; 
    else
        localNorm = localRegion;  % (all zeros? unlikely, but safe)
    end

    %   3) Convert to dB inside that window:
    rampBlock_dB = 10*log10(localNorm + eps);

    %   4) Threshold at a fixed dB (e.g. –22 dB below the local max → anything 
    %      ≥ –22 dB in this local window is considered a detection):
 
    rampDetections = (rampBlock_dB >= threshold_dB);
    %—————————————————————————————
    ...
    % NEW: Compute fraction of “1” pixels in the detection block
    fracDetected = nnz(rampDetections) / numel(rampDetections); %num non zeroes/ num elements
    
    % If more than 85% of the box is lit up, it’s almost certainly noise
    if fracDetected > 0.85
        vehicleClasses{k} = "Noise";
        normWidth(k)      = NaN;
        fprintf("Vehicle %d: >%.0f%% coverage → marking as Noise\n", k, fracDetected*100);
        continue;
    end
...
% Drop detections classifieed as noise (due to fragmented track producing two bounding boxes) before printing
vehicleClasses = string(vehicleClasses);                 % convert cell/char -> string
validIdx = vehicleClasses ~= "Noise" & ~ismissing(vehicleClasses);


vehicleClasses = vehicleClasses(validIdx);
allSpeeds      = allSpeeds(validIdx);
detWidth       = detWidth(validIdx);
normWidth      = normWidth(validIdx);
validTracks    = validTracks(validIdx);
repIndices     = repIndices(validIdx);

numVehicles = sum(validIdx);  % recompute total
